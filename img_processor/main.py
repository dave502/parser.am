...
def detect_faces(img_url: str):
    ...
    cv2.imwrite(str(new_file_path), image)
...
# загружает картинку
def save_processed_img(img_arr, path = './img/', ):
    tf.keras.utils.save_img(
        path, img_arr, data_format=None
    )
...
def run_detector(detector, path, max_boxes=20):
  ...
  path = "https://img.freepik.com/free-photo/blogger-journalist-young-cute-blonde-girl-recording-video-on-computer-in-office-lovely-sitting_140725-166040.jpg"  
  img = load_img(path)
  ...
  save_processed_img(image_with_boxes)
  return result  
