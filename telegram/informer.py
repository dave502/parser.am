...
class Informer:
    ...
    def notification(doc, date_updated: bool = False) -> str:
        new_notification_text = f"Уважаемый клиент(подписчик), настоящим сообщением уведомляем Вас, " \
                                ...
                                f"Требования к Замечаниям к проекту отчета согласно Статье 14 Федерального закона N 237-ФЗ \"" \
                                f"О государственной кадастровой оценке\" от 03.07.2016:\n"\
                                f"Замечания к проекту отчета могут быть представлены любыми лицами.\n"\
                                f"Замечание к проекту отчета наряду с изложением его сути должно содержать:\n"\
                                f" 1) фамилию, имя и отчество (последнее - при наличии) физического лица, полное наименование юридического "\
                                f"лица, номер контактного телефона, адрес электронной почты (при наличии) лица, представившего замечание "\
                                f"к проекту отчета;\n"\
                                f" 2) кадастровый номер объекта недвижимости, в отношении определения кадастровой стоимости которого "\
                                f"представляется замечание к проекту отчета, если замечание относится к конкретному объекту недвижимости;\n"\
                                f" 3) указание на номера страниц (разделов) проекта отчета, к которым представляется замечание (при необходимости)."
        date_updated_notification_text = f"Уважаемый клиент(подписчик), настоящим сообщением уведомляем Вас, " \
                                        ...
                                        f"(срок приёма замечаний продлён). "\
                                        f"Обновлённая версия проекта отчёта размещена на сайте государственного бюджетного учреждения по адресу:\n" \
                                        f"{doc.get('report_intermediate_docs_link')}.\n" \
                                        f"Дата размещения проекта отчёта: {doc.get('report_project_date_start')} \n" \
                                        f"Дата окончания срока ознакомления с проектом отчета: {doc.get('report_project_date_end')}\n" \
                                        f"Дата окончания приема замечаний к проекту отчета: {doc.get('report_project_date_end')}\n" \
